 ...
 * @return {Node}
 */function getAttrib1(){var type=NodeType.AttribType;var token=tokens[pos];var line=token.ln;var column=token.col;var content=[];pos++;content = content.concat(getSC()).concat([getIdent()]).concat(getSC()).concat([getAttrselector()]).concat(getSC()).concat([checkString(pos)?getString():getIdent()]).concat(getSC());var end=getLastPosition(content,line,column + 1,1);pos++;return newNode(type,content,line,column,end);} /**
 * Check if token is part of an attribute selector of the form `[attr]`
 * Attribute can not be empty, e.g. `[]`.
 * @param {Number} i Token's index number
 * @return {Number}
 */function checkAttrib2(i){var start=i;var l=undefined;if(i++ >= tokensLength)return 0;if(l = checkSC(i))i += l;if(l = checkIdent(i))i += l;else return 0;if(l = checkSC(i))i += l;return tokens[i].type === TokenType.RightSquareBracket?i - start:0;} /**
 * Get node with an attribute selector of the form `[attr]`
 * @return {Node}
 */function getAttrib2(){var type=NodeType.AttribType;var token=tokens[pos];var line=token.ln;var column=token.col;var content=[];pos++;content = content.concat(getSC()).concat([getIdent()]).concat(getSC());var end=getLastPosition(content,line,column + 1,1);pos++;return newNode(type,content,line,column,end);} /**
 ...
 *      `*=` or `|=`)
 * @return {Node}
 */function getAttrselector(){var type=NodeType.AttrselectorType;var token=tokens[pos];var line=token.ln;var column=token.col;var content=tokens[pos].value;pos++;if(tokens[pos] && tokens[pos].type === TokenType.EqualsSign)content += tokens[pos++].value;return newNode(type,content,line,column);} /**
 ...
 * @return {Node}
 */function getAtruleb(){var type=NodeType.AtrulebType;var token=tokens[pos];var line=token.ln;var column=token.col;var content=[getAtkeyword()].concat(getTsets()).concat([getBlock()]);return newNode(type,content,line,column);} /**
 ...
 * @return {Node}
 */function getAtruler(){var type=NodeType.AtrulerType;var token=tokens[pos];var line=token.ln;var column=token.col;var content=[getAtkeyword(),getAtrulerq()];content.push(getAtrulers());return newNode(type,content,line,column);} /**
 * @param {Number} i Token's index number
 * @return {Number}
 */function checkAtrulerq(i){return checkTsets(i);} /**
 * @return {Node}
 */function getAtrulerq(){var type=NodeType.AtrulerqType;var token=tokens[pos];var line=token.ln;var column=token.col;var content=getTsets();return newNode(type,content,line,column);} /**
 * @param {Number} i Token's index number
 * @return {Number}
 ...
 * @return {Node}
 */function getAtrulers(){var type=NodeType.AtrulersType;var token=tokens[pos++];var line=token.ln;var column=token.col;var content=getSC();while(!tokens[pos].atrulers_end) {var childType=tokens[pos].atrulers_child;if(childType === 1)content = content.concat(getSC());else if(childType === 2)content.push(getAtrule());else if(childType === 3)content.push(getRuleset());}content = content.concat(getSC());var end=getLastPosition(content,line,column,1);pos++;return newNode(type,content,line,column,end);} /**
 * @param {Number} i Token's index number
 * @return {Number}
 ...
 * @return {Node}
 */function getAtrules(){var type=NodeType.AtrulesType;var token=tokens[pos];var line=token.ln;var column=token.col;var content=[getAtkeyword()].concat(getTsets());return newNode(type,content,line,column);} /**
 ...
 * @return {Node}
 */function getCombinator(){var type=NodeType.CombinatorType;var token=tokens[pos];var line=token.ln;var column=token.col;var content=tokens[pos++].value;return newNode(type,content,line,column);} /**
 ...
 * @return {Node}
 */function getFunction(){var type=NodeType.FunctionType;var token=tokens[pos];var line=token.ln;var column=token.col;var ident=getIdent();var content=[ident];content.push(ident.content === 'not'?getNotArguments():getArguments());return newNode(type,content,line,column);} /**
 * @return {Node}
 ...
 * @return {Node}
 */function getNotArguments(){var type=NodeType.ArgumentsType;var token=tokens[pos];var line=token.ln;var column=token.col;var content=[];pos++;while(pos < tokensLength && tokens[pos].type !== TokenType.RightParenthesis) { // TODO: Remove these checks
 ...
 * @return {Node}
 */function getImportant(){var type=NodeType.ImportantType;var token=tokens[pos++];var line=token.ln;var column=token.col;var content=getSC();var end=getLastPosition(content,line,column,9);pos++;return newNode(type,content,line,column,end);} /**
 ...
 * @return {Node}
 */function getNth(){var type=NodeType.NthType;var token=tokens[pos];var line=token.ln;var column=token.col;var content=undefined;if(token.nth_last){content = joinValues(pos,token.nth_last);pos = token.nth_last + 1;}else {content = token.value;pos++;}return newNode(type,content,line,column);} /**
 ...
 * @return {Node}
 */function getNthselector(){var type=NodeType.NthselectorType;var token=tokens[pos];var line=token.ln;var column=token.col;var content=[];var nthf=newNode(NodeType.IdentType,getNthf(),line,column + 1);content.push(nthf);pos++;while(tokens[pos].type !== TokenType.RightParenthesis) {var childType=tokens[pos].nthselector_child;if(childType === 1)content = content.concat(getSC());else if(childType === 2)content.push(getUnary());else if(childType === 3)content.push(getNth());}var end=getLastPosition(content,line,column,1);pos++;return newNode(type,content,line,column,end);} /**
 ...
 * @return {Node}
 */function getPseudoc(){var type=NodeType.PseudocType;var token=tokens[pos++];var line=token.ln;var column=token.col;var content=[];var childType=tokens[pos].pseudoc_child;if(childType === 1)content.push(getFunction());else content.push(getIdent());return newNode(type,content,line,column);} /**
 * @param {Number} i Token's index number
 * @return {Number}
 ...
 * @return {Node}
 */function getRuleset(){var type=NodeType.RulesetType;var token=tokens[pos];var line=token.ln;var column=token.col;var content=[];while(pos < tokensLength) {var childType=tokens[pos].ruleset_child;if(childType === 1){content.push(getBlock());break;}else if(childType === 2)content.push(getSelector());else break;}return newNode(type,content,line,column);} /**
 ...
 */function checkSelector(i){var start=i;var l=undefined;while(i < tokensLength) {if(l = checkDelim(i))tokens[i].selector_child = 1;else if(l = checkSimpleSelector(i))tokens[i].selector_child = 2;else break;i += l;}if(i !== start)tokens[start].selector_end = i - 1;return i - start;} /**
 * @return {Node}
 */function getSelector(){var type=NodeType.SelectorType;var token=tokens[pos];var line=token.ln;var column=token.col;var content=[];var selector_end=token.selector_end;while(pos <= selector_end) {var childType=tokens[pos].selector_child;if(childType === 1)content.push(getDelim());else if(childType === 2)content.push(getSimpleSelector());}return newNode(type,content,line,column);} /**
 ...
 *      selector
 * @return {Node}
 */function getShash(){var type=NodeType.ShashType;var token=tokens[pos];var line=token.ln;var column=token.col;var content=[];pos++;var ln=tokens[pos].ln;var col=tokens[pos].col;var ident=newNode(NodeType.IdentType,getNmName(),ln,col);content.push(ident);return newNode(type,content,line,column);} /**
 * @param {Number} i Token's index number
 * @return {Number}
 */function checkSimpleSelector(i){var start=i;var l=undefined;while(i < tokensLength) {if(l = checkSimpleSelector1(i))i += l;else break;}tokens[start].simpleselector_end = i;return i - start;} /**
 * @return {Node}
 */function getSimpleSelector(){var type=NodeType.SimpleselectorType;var token=tokens[pos];var line=token.ln;var column=token.col;var content=[];var end=token.simpleselector_end;var t=undefined;while(pos < end) {t = getSimpleSelector1();if(typeof t.content === 'string')content.push(t);else content = content.concat(t);}return newNode(type,content,line,column);} /**
 * @param {Number} i Token's index number
 * @return {Number}
 ...
 *      converted to string.
 */function getUnary(){var type=NodeType.UnaryType;var token=tokens[pos];var line=token.ln;var column=token.col;var content=token.value;pos++;return newNode(type,content,line,column);} /**
